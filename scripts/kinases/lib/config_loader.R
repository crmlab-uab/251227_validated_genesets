# config_loader.R - Centralized configuration and path management for kinases pipeline
# Author: bRNA3F AI Agent
# Created: 2025-12-31
# Purpose: Provide consistent config loading and path resolution for all pipeline scripts
#
# Usage:
#   source("lib/config_loader.R")  # from scripts/kinases/
#   # OR
#   source("scripts/kinases/lib/config_loader.R")  # from repo root
#
...
# Define canonical paths relative to repo_root
paths <- list(
  input_dir = file.path(repo_root, cfg_get("input_dir", "curated/kinases/inputs")),
  output_dir = file.path(repo_root, cfg_get("output_dir", "curated/kinases/outputs")),
  cache_dir = file.path(repo_root, cfg_get("hgnc.cache", "cache"))
)
...
# Print configuration summary (can be suppressed with KINASES_QUIET=1)
if (Sys.getenv("KINASES_QUIET", "0") != "1") {
