#!/usr/bin/env Rscript
# Comprehensive Kinase Validation using UniProt, GO, and Bioconductor
# Author: Bioinformatics Pipeline
# Date: 2025-12-26
# Purpose: Independently validate all 540 kinases against multiple databases
...
kinases <- fread("data/genesets/custom/201006_composite_kinases_curated.csv", header = FALSE)
colnames(kinases) <- c(
...
validate_kinase <- function(mouse_symbol, human_uniprot, entrez_id, row_idx) {
  ...
  if (!is.na(entrez_id) && entrez_id != "") {
    ...
    if (!is.na(human_uniprot) && human_uniprot != "") {
      # Query UniProt for protein function
      uniprot_data <- tryCatch({
        UniProt.ws::select(
          up,
          keys = human_uniprot,
          columns = c("PROTEIN-NAMES", "KEYWORDS", "GO-ID"),
          keytype = "UNIPROTKB"
        )
      ...
      if (!is.null(uniprot_data) && nrow(uniprot_data) > 0) {
        ...
        if (!is.na(uniprot_data$KEYWORDS[1])) {
          if (grepl("kinase|Kinase", uniprot_data$KEYWORDS[1], ignore.case = TRUE)) {
        ...
        # Check protein names
        if (!is.na(uniprot_data$`PROTEIN-NAMES`[1])) {
          if (grepl("kinase|Kinase", uniprot_data$`PROTEIN-NAMES`[1], ignore.case = TRUE)) {
            result$UniProt_Kinase <- TRUE
            result$Validation_Evidence <- ifelse(
              result$Validation_Evidence == "GO",
              "GO+UniProt",
...
output_file <- "data/genesets/custom/mouse_kinome_comprehensive_validation.csv"
...
writeLines(summary_report, "data/genesets/custom/VALIDATION_REPORT.md")
cat("✓ Summary report saved to: data/genesets/custom/VALIDATION_REPORT.md\n")
